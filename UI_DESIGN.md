...
- Web 服务可用时，欢迎内容末尾以独立入口展示浏览器地址：`accent` 标识、`cyan` 加粗地址及中性辅助说明。地址在窄屏中换行，不把装饰前缀写入可复制的 URL，也不把入口写入模型上下文。
...
方向键在可用选项间移动，Enter 执行当前项。第 `2`、`3` 项仅适用于非 Root、无需强确认且最高为 `Mutating` 的操作；Root、Dangerous、Critical 或强确认操作必须显示为不可用且跳过选择。任务级许可只按完整命令精确匹配，仅在当前自然语言任务的 Agent loop 内存中存在。本次运行许可只保存在当前进程内存中，可由 `/permission allow` 启用、由 `/permission ask` 关闭；它不持久化、不进入会话，且永远不覆盖 Root、Dangerous、Critical 或强确认。即使 UI 返回了不合法的许可决策，执行边界也必须再次执行上述限制。
...
adb 或宿主终端可能把方向键 CSI 序列拆成 `Esc`、`[`、`A/B` 等事件。初始审批阶段必须忽略孤立 `Esc` 和大写字母 CSI 尾字符，只接受小写 `y/n/a/r/e/i/t` 与数字快捷键，避免方向键被误判为拒绝或许可操作。强确认和编辑阶段仍可使用 `Esc` 取消。
...
- Web 工具结果默认折叠，点击标题可独立切换；对话页的 F2 将全部工具卡片统一展开或收起，若当前有任一卡片折叠则先全部展开。审批/终端弹窗打开时不拦截 F2。
