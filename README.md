...
默认配置位于解析符号链接后的可执行文件目录，名称为 `config.toml`。配置不存在且以普通 TUI 模式启动时，程序会用 2 秒超时探测 KONKA 内部便捷体验服务；若收到任意 HTTP 响应，则以 `0600` 权限自动创建使用 `deepseek-v4-flash-0731` 模型和 Responses 协议的 `KK-FREE-TEST` 配置。已有配置文件绝不参与探测或改写；内网服务不可达时仍直接进入 TUI，可使用 `/config` 或 `/setting` 打开统一设置面板。旧的逐行配置向导和 `--init` 已移除。配置完成前普通任务不会发送给模型，也可传入 `--config /path/config.toml`。
