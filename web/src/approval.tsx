...
export function Approval({id,pending,done,stop}:{id:string;pending:Pending;done:()=>void;stop:()=>void}){
  ...
  const dialog=useModalFocus(()=>{if(!busy)void decide(pending.kind==='approval'?'reject':'cancel')});
  return <div class="backdrop"><div ref={dialog} tabIndex={-1} class="modal approval" role="dialog" aria-modal="true" aria-label={pending.kind==='approval'?'执行审批':'补充信息'}><h2>{pending.kind==='approval'?'确认设备操作':'补充信息'}</h2>{pending.kind==='approval'?<><p class="approval-impact">{riskDescription(pending)}</p><p>风险等级：{riskName(pending.risk)}{pending.root?' · 需要 Root 权限':''}{pending.strong?' · 需要强确认':''}</p>{pending.explanation&&<p>检查依据：{pending.explanation}</p>}<p class="muted">拒绝后不会执行这一步。批准仅对当前显示的操作有效。按 Esc 可拒绝。</p><strong>待执行内容</strong><pre class="approval-preview">{pending.command}</pre><details><summary>编辑待执行内容</summary><label>修改后会重新检查风险<textarea value={edit} onInput={e=>setEdit(e.currentTarget.value)}/></label><button disabled={busy||edit===pending.command} onClick={()=>decide('edit',edit)}>提交编辑并重新检查风险</button></details>{pending.strong&&<label>确认高风险操作：输入 CONFIRM<input value={confirm} onInput={e=>setConfirm(e.currentTarget.value)} autocomplete="off" placeholder="CONFIRM"/></label>}<div class="actions"><button class="primary" disabled={busy||(pending.strong&&confirm!=='CONFIRM')} onClick={()=>decide('approve',pending.strong?confirm:undefined)}>批准这一次</button>{!pending.strong&&!pending.root&&<button disabled={busy} onClick={()=>decide('remember')}>本任务记住同一操作</button>}<button disabled={busy} onClick={()=>decide('reject')}>拒绝这一步</button><button disabled={busy} onClick={stop}>停止整个任务</button></div></>:<>{pending.questions?.map(q=><label key={q.id}>{q.prompt}<input list={`q-${q.id}`} onInput={e=>setAnswers({...answers,[q.id]:e.currentTarget.value})}/><datalist id={`q-${q.id}`}>{q.options.map(o=><option value={o.value}>{o.label}</option>)}</datalist></label>)}<div class="actions"><button disabled={busy} onClick={()=>decide('answer')}>提交</button><button disabled={busy} onClick={()=>decide('cancel')}>取消补充信息</button><button disabled={busy} onClick={stop}>停止整个任务</button></div></>}{error&&<p class="bad" role="alert">{error}</p>}</div></div>;
}
