...
export interface Snapshot{id:string;title:string;entries:Entry[];busy:boolean;pending:Pending|null;turns:number;steps:number;tool_calls:number;input_tokens:number;output_tokens:number;final_input_tokens?:number;activity:'idle'|'thinking'|'tool'|'waiting'|'cancelling';activity_detail?:string|null;activity_elapsed_ms:number;received_at_ms?:number}
...
export interface ToolInfo{name:string;description:string;category:string;risk:string}
